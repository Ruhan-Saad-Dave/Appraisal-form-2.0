...
import { DEAN_TRACKS, getSchoolByValue, getSchoolKey, getSchoolsByDeanTrack, schoolVisualMeta } from "../../../constants/universityHierarchy";
...
{!isStandardAppraisalSchool(approval?.school || approval?.schoolName || approval?.info?.school || "") && (
<ReviewTable
...
function StandardApprovalReviewPanel({ approval, approvalType, onBack, onSubmit, readOnly = false }) {
 ...
 const subjectSchoolConfig = getSchoolByValue(approval.school || approval.schoolName || approval.info?.school || "");
 const facultyChainIncludesHod = subjectRole === "faculty" && Boolean(subjectSchoolConfig?.hasHod || subjectSchoolConfig?.approvalChain?.includes("hod"));
...
/>
 ...
 const useFacultyDeanSummaryRows = subjectRole === "faculty" && !facultyChainIncludesHod;
 useEffect(() =>{
 let active = true;
...
{canReject && (
<button onClick={async () =>{ if (await confirmRejection("Reject this appraisal and send it back to the user for editing?", "reject")) { onSubmit(approval.id, displayedDeanScores, remarks, sectionScores, reviewConfirmed, "rejected"); } }}
 disabled={!reviewConfirmed || !remarks.trim()}
                      ...
                      <button
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            // Non-engineering can mix Standard and Creative form families based
                            // on each school's config. Running a Standard submission through the
                            // Creative normalizer needlessly exposes it to that engine's own
                            // field-aliasing per section.
